...
class Test_Sendbird:
    ...
    def test_get_threats(self, mock_get_data: MagicMock) -> None:
        ...
        mock_get_data.return_value = _mocked_get_data_value
    ...
    @respx.mock
    @patch("blocket_api.sendbird.SendbirdClient.get_data")
    def test_unread_messages_count(self, mock_get_data: MagicMock) -> None:
        respx.get(f"{BASE_URL}/messaging-sendbird-communication/v1/users/tokens").mock(
            return_value=Response(
                status_code=200,
                json={
        ...
        respx.get(
            f"{SENDBIRD_URL}/v3/users/c1-user-id/unread_message_count?super_mode=all&include_feed_channel=false"
        ).mock(
            return_value=Response(
                status_code=200,
                json={"unread_count": 5},
        ...
        assert api.unread_messages_count() == {"unread_count": 5}
        assert api.unread_messages_count(as_objects=True) == UnreadMessagesCount(
            unread_count=5
        )
