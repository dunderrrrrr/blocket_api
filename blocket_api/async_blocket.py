...
from .blocket import QueryParam
...
def _build_params(param_dict: dict[str, str | int | None]) -> list[QueryParam]:
    return [QueryParam(k, v) for k, v in param_dict.items() if v is not None]
...
class AsyncBlocketAPI:
    ...
    async def search(
        ...
        *,
        ...
        params = _build_params(param_dict)
    ...
    async def get_ad(
        ...
        response = await self.client.get(ad.url, headers=HEADERS)
        response.raise_for_status()
